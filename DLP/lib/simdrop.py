...
""" def drop_consecutive_layers(model, start: int, n: int, is_opt: bool = False):
    
    #[start, start+n) 레이어를 제거하여 깊이를 줄인다. config.num_hidden_layers도 동기화.
    ...
    keep_idx = [i for i in range(len(layers)) if not (start <= i < start + n)]
...
def drop_consecutive_layers(model, ell: int, n: int, is_opt: bool):
    """
    기존: 레이어를 pop/remove
    변경: 해당 구간 레이어를 LlamaPassLayer로 치환 (구조 보존)
    반환: removed_indices (어떤 레이어를 치환했는지)
    """
    layers = model.model.decoder.layers if is_opt else model.model.layers
