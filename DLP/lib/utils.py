...
def prepare_calibration_input_opt(model, dataloader, device):
    ...
    if isinstance(dm, dict) and "model.embed_tokens" in dm:
        dev = dm["model.embed_tokens"]
    else:
        # 샤딩 안 했으면 호출자가 준 device(또는 모델 파라미터의 디바이스)를 사용
        dev = device  # 또는: next(model.parameters()).device
...
def prepare_calibration_input(model, dataloader, device):
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, inp, **kwargs):
            inps[cache['i']] = inp
            cache['i'] += 1
            cache['attention_mask'] = kwargs.get('attention_mask', None)  # ← get
            cache['position_ids'] = kwargs.get('position_ids',   None)  # ← get
            raise ValueError
