...
def main():
    ...
    # ★ AB Adapter 적용 개선: inference_mode=True
    with wall_timer("AB Adapter 적용"), gpu_timer("AB Adapter 적용"):
        logger.info("AB 어댑터 적용")
        ...
        model = get_peft_model(model, lora_config, adapter_name="adapter_AB")
        ...
        model.set_adapter("adapter_AB")
        
        # ★ 로드 결과 분석 개선
        lora_params = [k for k in fixed_state_dict.keys() if 'lora_' in k]
        lora_missing = [k for k in missing_keys if 'lora_' in k]
        lora_unexpected = [k for k in unexpected_keys if 'lora_' in k]
        
        logger.info(f"LoRA 로드 완료 - Loaded: {len(lora_params)}, Missing: {len(lora_missing)}, Unexpected: {len(lora_unexpected)}")
        
        if lora_missing:
            logger.warning(f"Missing LoRA parameters: {len(lora_missing)}")
            logger.warning(f"First few: {lora_missing[:3]}")
        
        if lora_unexpected:
            logger.warning(f"Unexpected LoRA parameters: {len(lora_unexpected)}")
            logger.warning(f"First few: {lora_unexpected[:3]}")
    
    log_mem("after_AB_lora_load")
    
    # ★ 어댑터 활성화 확인
    with wall_timer("어댑터 활성화 확인"):
        is_active = check_activation(model)
        logger.info(f"AB 어댑터 활성: {is_active}")
        
        if not is_active:
            logger.error("⚠️ AB 어댑터가 활성화되지 않았습니다!")
        else:
            logger.info("✓ AB 어댑터 정상 활성화")
